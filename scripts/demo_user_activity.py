...
from konnect.agents.recommendation import get_user_activity, RecommendationAgent
...
def create_demo_data():
    ...
    try:
        ...
        listing_items = [
            ...
            {"title": "Scientific Calculator", "price": 45.0, "category": "Electronics"},
        ...
        activities = []
        activity_items = [
            {"type": "view", "target": listings[0], "data": {"duration": 45, "source": "search"}},
            {"type": "view", "target": listings[2], "data": {"duration": 30, "source": "category"}},
            {"type": "search", "target": None, "data": {"query": "laptop electronics"}},
            {"type": "view", "target": listings[4], "data": {"duration": 20, "source": "recommendations"}},
            {"type": "search", "target": None, "data": {"query": "study materials books"}},
        ]
...
def demonstrate_user_activity_tool(user_id):
    ...
    print(f"  • Favorite Categories: {', '.join(activity_data['favorite_categories']) or 'None yet'}")
    
    print(f"\n💰 Recent Purchases ({len(activity_data['recent_purchases'])}):")
    for purchase in activity_data['recent_purchases']:
        print(f"  • Listing ID {purchase['listing_id']}: ${purchase['amount']:.2f} ({purchase['status']})")
    
    print(f"\n👀 Recent Activities ({len(activity_data['recent_activities'])}):")
    for activity in activity_data['recent_activities']:
        target_info = f"target {activity['target_id']}" if activity['target_id'] else "general"
    ...
    for search in activity_data['recent_searches']:
        search_data = json.loads(search['activity_data']) if search['activity_data'] else {}
